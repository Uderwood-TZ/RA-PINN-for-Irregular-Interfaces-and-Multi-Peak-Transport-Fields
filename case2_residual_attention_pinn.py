...
CASE_NAME = "case2_residual_attention_pinn"
...
def sech(z: torch.Tensor) -> torch.Tensor:
    return 1.0 / torch.cosh(z)


def build_stream_function(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    layer = torch.tanh(PSI_BETA_X * x)
    core = sech(PSI_BETA_S * x)
    psi = PSI_A * layer * torch.sin(math.pi * y)
    psi = psi + PSI_B * core * torch.sin(2.0 * math.pi * y) * torch.cos(math.pi * x)
    return psi
...
def exact_solution_from_xy(xy: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    neutral_core = sech(PRESSURE_BETA_1 * x) * torch.cos(math.pi * y)
    p = PRESSURE_A * torch.tanh(PRESSURE_BETA_2 * x)
    p = p + PRESSURE_B * neutral_core
    p = p + PRESSURE_C * torch.sin(math.pi * x) * torch.sin(math.pi * y)

    T = TEMP_BASE + TEMP_A * sech(TEMP_BETA_X * x) * torch.cos(math.pi * y)
    T = T + TEMP_B * torch.sin(math.pi * x) * torch.sin(2.0 * math.pi * y)

    phi = PHI_A * sech(PHI_BETA_1 * x) * torch.cos(math.pi * y)
    phi = phi + PHI_B * torch.tanh(PHI_BETA_2 * x) * torch.sin(2.0 * math.pi * y)
    phi = phi + PHI_C * sech(0.55 * PHI_BETA_1 * x) * torch.cos(2.0 * math.pi * y)
...
Case-2 manufactured solution: bipolar steep-gradient / neutral-zone coupled EHD field.
Core design target: a strong compound gradient concentrated near x = 0 with transverse sign variation in y.
Representative bipolar profile: sech(beta x) * cos(pi y).

Exact MMS used here:
psi(x, y) = {PSI_A} * tanh({PSI_BETA_X} x) * sin(pi y)
             + {PSI_B} * sech({PSI_BETA_S} x) * sin(2 pi y) * cos(pi x)
u = d psi / d y, v = - d psi / d x

p(x, y) = {PRESSURE_A} * tanh({PRESSURE_BETA_2} x)
          + {PRESSURE_B} * sech({PRESSURE_BETA_1} x) * cos(pi y)
          + {PRESSURE_C} * sin(pi x) * sin(pi y)

T(x, y) = {TEMP_BASE} + {TEMP_A} * sech({TEMP_BETA_X} x) * cos(pi y)
          + {TEMP_B} * sin(pi x) * sin(2 pi y)

phi(x, y) = {PHI_A} * sech({PHI_BETA_1} x) * cos(pi y)
            + {PHI_B} * tanh({PHI_BETA_2} x) * sin(2 pi y)
            + {PHI_C} * sech({0.55 * 7.5:.3f} x) * cos(2 pi y)

This script uses data loss, boundary loss and PDE residual loss with 7:3 train/validation split for supervised MMS data.
All field maps, absolute error maps, max-error maps, metrics, timing logs and x-y-value text exports are saved separately.
""".strip()
