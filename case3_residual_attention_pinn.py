...
CASE_NAME = "case3_residual_attention_pinn"
...
PHI_COEFFS = [(1, 0.90), (2, 0.35), (3, 0.22)]
...
def rescaled_coords(x: torch.Tensor, y: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor]:
    xr = 0.5 * (x + 1.0)
    yr = 0.5 * (y + 1.0)
    return xr, yr


def multi_gaussian_charge(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    q = torch.zeros_like(x)
    for cx, cy, amp in CHARGE_PEAKS:
        q = q + amp * torch.exp(-((x - cx) ** 2 + (y - cy) ** 2) / (2.0 * GAUSS_SIGMA ** 2))
    q = q / max(amp for _, _, amp in CHARGE_PEAKS)
    q = q + 0.12 * torch.sin(math.pi * x) * torch.cos(2.0 * math.pi * y)
    return q


def build_stream_function(x: torch.Tensor, y: torch.Tensor) -> torch.Tensor:
    q = multi_gaussian_charge(x, y)
    xr, yr = rescaled_coords(x, y)
    psi = 0.20 * q * torch.sin(math.pi * xr) * torch.sin(math.pi * yr)
    psi = psi + 0.08 * torch.sin(2.0 * math.pi * xr) * torch.sin(math.pi * yr)
    psi = psi + 0.05 * torch.sin(math.pi * xr) * torch.sin(3.0 * math.pi * yr)
    return psi
...
def exact_solution_from_xy(xy: torch.Tensor) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor, torch.Tensor]:
    ...
    for n, amp in PHI_COEFFS:
        phi = phi + amp * torch.sin(n * math.pi * xr) * torch.sin(n * math.pi * yr)
    q = multi_gaussian_charge(x, y)
    phi = phi + 0.18 * q

    p = 0.50 * q
    p = p + 0.20 * torch.sin(2.0 * math.pi * xr) * torch.cos(math.pi * yr)
    p = p + 0.10 * torch.cos(3.0 * math.pi * xr) * torch.sin(2.0 * math.pi * yr)

    T = 0.45 + 0.55 * q
    T = T + 0.15 * torch.sin(math.pi * xr) * torch.sin(2.0 * math.pi * yr)
    T = T + 0.10 * phi
...
def compute_mms_sources(xy: torch.Tensor) -> torch.Tensor:
    ...
    q = multi_gaussian_charge(xy[:, 0:1], xy[:, 1:2])

    f_u = u * d["u_x"] + v * d["u_y"] + d["p_x"]
    f_u = f_u - VISCOSITY_NU * (d["u_xx"] + d["u_yy"])
    f_u = f_u - ELECTRIC_FORCE_COEFF * phi * d["phi_x"] - THERMAL_FORCE_COEFF * T * d["T_x"]

    f_v = u * d["v_x"] + v * d["v_y"] + d["p_y"]
    f_v = f_v - VISCOSITY_NU * (d["v_xx"] + d["v_yy"])
    f_v = f_v - ELECTRIC_FORCE_COEFF * phi * d["phi_y"] - THERMAL_FORCE_COEFF * T * d["T_y"]

    q_T = u * d["T_x"] + v * d["T_y"]
    q_T = q_T - THERMAL_DIFFUSIVITY * (d["T_xx"] + d["T_yy"])
    q_T = q_T - JOULE_HEATING_COEFF * (d["phi_x"] ** 2 + d["phi_y"] ** 2)

    s_phi = d["phi_xx"] + d["phi_yy"] - PHI_REACTION_COEFF * phi - q
...
class ResidualAttentionPINN(nn.Module):
    def __init__(self, in_dim: int = 2, hidden_dim: int = HIDDEN_DIM, num_blocks: int = NUM_BLOCKS) -> None:
        super().__init__()
        self.input_linear = nn.Linear(in_dim, hidden_dim)
        self.blocks = nn.ModuleList([ResidualAttentionBlock(hidden_dim) for _ in range(num_blocks)])
    ...
    def forward(self, xy: torch.Tensor) -> torch.Tensor:
        h = torch.tanh(self.input_linear(xy))
...
def pde_residuals(model: nn.Module, xy: torch.Tensor, src: torch.Tensor) -> Dict[str, torch.Tensor]:
    ...
    r_momx = r_momx - ELECTRIC_FORCE_COEFF * phi * d["phi_x"] - THERMAL_FORCE_COEFF * T * d["T_x"] - src_u

    r_momy = u * d["v_x"] + v * d["v_y"] + d["p_y"]
    r_momy = r_momy - VISCOSITY_NU * (d["v_xx"] + d["v_yy"])
    r_momy = r_momy - ELECTRIC_FORCE_COEFF * phi * d["phi_y"] - THERMAL_FORCE_COEFF * T * d["T_y"] - src_v
    ...
    q = multi_gaussian_charge(xy[:, 0:1], xy[:, 1:2])
    r_phi = d["phi_xx"] + d["phi_yy"] - PHI_REACTION_COEFF * phi - q - src_phi
...
1) u_x + v_y = 0
2) u u_x + v u_y + p_x - nu (u_xx + u_yy) - c_e phi phi_x - c_T T T_x = f_u
3) u v_x + v v_y + p_y - nu (v_xx + v_yy) - c_e phi phi_y - c_T T T_y = f_v
4) u T_x + v T_y - alpha (T_xx + T_yy) - gamma_J (phi_x^2 + phi_y^2) = q_T
5) phi_xx + phi_yy - lambda_phi phi - q(x, y) = s_phi

Exact MMS:
Case3 uses a multi-peak charge migration field q(x, y) built from several Gaussian peaks plus a sinusoidal perturbation.
The stream function psi, pressure p, temperature T and electric potential phi are defined in the script and used to construct the source terms.
